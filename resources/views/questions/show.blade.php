...
<main class="max-w-4xl mx-auto px-4 py-10">
    <div class="bg-white p-8 rounded-xl shadow-lg">
        ...
        <div class="mb-6 pb-4 border-b border-gray-200">
            ...
            <p class="mt-4 text-gray-700 leading-relaxed">{{ $question->content }}</p>
        </div>
        ...
        <div class="mt-10 pt-6 border-t border-gray-200">
            ...
            <form action="{{ route('answers.store', $question->id) }}" method="POST">
                ...
                <div class="mb-3">
                    <textarea 
                        name="content" 
                        class="w-full p-3 border rounded-lg focus:ring-blue-500 focus:border-blue-500 @error('content') border-red-500 @enderror" 
                        rows="4" 
                        placeholder="Tulis jawaban terbaik Anda di sini..."
                        required>{{ old('content') }}</textarea>
                    @error('content')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <button type="submit" class="bg-blue-600 text-white font-semibold px-6 py-2 rounded-lg hover:bg-blue-700 transition active:scale-95">
                    Kirim Jawaban
                </button>
            </form>
        </div>
